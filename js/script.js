        ...
        for (let mutation of mutationsList) {
            if (mutation.type === 'childList') {
                ...
                if (professionElement && !document.querySelector('.multiple-text')) {
                    ...
                    // Reinicializa Typed.js
                    loadTranslationsAndInitializeTyped('es'); // o cualquier idioma seleccionado actualmente
                }
            }
        }
    });
...
function applySpanColor() {
    document.querySelectorAll('span.texto-resaltado').forEach(span => {
        span.style.color = '#007BFF'; // Aplica el color azul
    });
}
...
function changeLanguage(lang) {
    ...
    // Destruir la instancia existente de Typed.js si existe
    if (typedInstance) {
        typedInstance.destroy();
        typedInstance = null; // Elimina la referencia
    }
    ...
    fetch(translationsPath)
        ...
        .then(translations => {
            ...
            // Reagregar el span si se ha eliminado
            const professionElement = document.querySelector('[data-lang="home.profession"]');
            if (professionElement && !document.querySelector('.multiple-text')) {
                console.log("Reagregando el elemento .multiple-text...");
                const span = document.createElement('span');
                span.className = 'multiple-text';
                professionElement.appendChild(span);
            }
            ...
            console.log("Inicializando Typed.js después del cambio de idioma");
            const stringsArray = translations.home.multiple_text;
            if (Array.isArray(stringsArray)) {
                initializeTyped(stringsArray);
            }
